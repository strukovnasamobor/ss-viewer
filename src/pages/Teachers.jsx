...
export default function Teachers() {
    ...
    const unsubscribeSchedulesTeachers = onSnapshot(
      ...
      (docSnap) => {
        if (docSnap.exists()) {
          ...
          if (newData === null) {
            console.log("Teachers > No usable new_data!");
            return;
          }
          const data = [];
          for (let i = 0; i < newData.length; i++) {
            data[newData[i].id] = {
              ...
              classteacherId: newData[i].classteacher,
              viceteacher: newData[i].viceteacher.split('').join('.'),
              viceteacherId: newData[i].viceteacher,
  ...
  useArrowKeyNavigation(
    !isTouchDevice && !!teacherId && (location.pathname === "/" || location.pathname.includes("teachers")) && !showColorPicker,
    navigateToCard
  );
  ...
  // razrednik / zamjenik in the header: link to the class when the teacher has one
  const renderHeaderClassLink = (classId, label) => {
    const id = classId?.trim();
    if (!id) return label;
    return (
      <span
        className="header-class-link clickable"
        onClick={() => router.push(`/classes?id=${id}`, "forward", "push")}
      >
        {label}
      </span>
    );
              ...
              <th colSpan={7}>
                <div className="table-header-container">
                  ...
                  <div className="header-container-right">
                    <div>
                      {t("classTeacher")}:&nbsp;{renderHeaderClassLink(teacherData.classteacherId, teacherData.classteacher)}<br />
                      {t("deputyClassTeacher")}:&nbsp;{renderHeaderClassLink(teacherData.viceteacherId, teacherData.viceteacher)}
                    </div>
